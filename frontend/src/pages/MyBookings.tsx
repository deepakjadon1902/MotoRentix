      ...
      <div className="container mx-auto max-w-4xl">
        ...
        {bookings.length === 0 ? (
          <div className="glass rounded-2xl p-12 text-center">
            ...
            <Link to="/dashboard" className="btn-primary-gradient px-6 py-3 rounded-lg text-primary-foreground font-semibold inline-block">
              Browse Vehicles
            </Link>
          </div>
        ) : (
          <div className="space-y-5">
            {bookings.map((booking, i) => {
              ...
              const className = statusMeta.className;
                  ...
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="flex-1 p-5 flex flex-col justify-between">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4">
                          <div className="bg-secondary/60 rounded-lg p-3">
                            ...
                            <p className="text-sm font-semibold text-foreground mt-0.5">
                              {booking.durationType === "hour" ? "Hourly" : "Daily"}
                            </p>
                          </div>
                          <div className="bg-secondary/60 rounded-lg p-3">
                            <span className="text-xs text-muted-foreground flex items-center gap-1">
                          ...
                          <div className="bg-secondary/60 rounded-lg p-3">
                            ...
                            <p className="text-sm font-semibold text-foreground mt-0.5">
                              INR {booking.durationType === "hour" ? vehicle.pricePerHour : vehicle.pricePerDay}
                              <span className="text-xs text-muted-foreground">
                                /{booking.durationType === "hour" ? "hr" : "day"}
                              </span>
                            </p>
                          </div>
                          <div className="bg-secondary/60 rounded-lg p-3">
                            ...
                            <p className="text-sm font-bold text-primary mt-0.5">INR {booking.totalPrice}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-4 pt-3 border-t border-border">
                        <span className="text-xs text-muted-foreground">
                          Booked on {booking.createdAt ? booking.createdAt.split("T")[0] : "-"}
                        </span>
                        <Link to={`/vehicle/${vehicle.id}`} className="text-sm font-medium text-primary hover:underline">
                          View Vehicle
                        </Link>
                      </div>
                    </div>
